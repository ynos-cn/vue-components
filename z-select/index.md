...
```html
<z-select :find-url="'/system/sys/role/find'" v-model:value="value">
</z-elect>
```
...
| 参数 | 说明 | 类型 | 默认值 | 版本 |
| --- | --- | --- | --- | --- |
| findUrl | `API`查询URL | string | - |  |
| apiCtx | `API`接口上下文 | string |  |  |
| bingLabel | 需要绑定的标题，显示的标题字段 | string | name |  |
| bingValue | 需要绑定的键，绑定的字段 | string | sid |  |
| multipleLabel | 是否需要展示多个标题 | boolean | false |  |
| bingMultipleLabel | 需要展示多个标题，显示的标题字段 | string | title |  |
| findPageSize | 查询大小 | number | 10 |  |
| dataPath | `API`数据相应的路径，常见的`body`或者`body.data` | string | body |  |
| paramBodyData | 条件查询 | object | {} |  |
| bingValueFindType | 绑定值查询类型 String Array | 'string'|'array' | 'string' |  |
| pushOptions | 往底部追加数据 | DefaultOptionType[] | - |  |
| unshiftOptions | 往顶部追加数据 | DefaultOptionType[] | - |  |
